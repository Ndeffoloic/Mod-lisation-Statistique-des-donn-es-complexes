...
def simulation_et_tests(p, file):
    ...
    file.write(f"Min p-value pour p={p}: {min_p_value}\n")
    file.write(f"Max p-value pour p={p}: {max_p_value}\n")
    
    plt.hist(p_values, bins=10, edgecolor='k')
    plt.xlabel('p-values')
    plt.ylabel('Frequency')
    plt.title(f'Histogram of p-values for p={p}')
    plt.savefig(f'histogram_p_{p}.png')
    plt.close()
    file.write(f"Histogram saved as histogram_p_{p}.png\n")
    ...
    file.write(f"Nombre de faux positifs pour p={p}: {false_positives}\n")
    ...
    file.write(f"L'erreur de type I empirique est {empirical_type_I_error:.2f}, ce qui est {comparison} alpha.\n")
...
with open('results.txt', 'w') as file:
    ...
    def test_linked_covariates(X, Y, alpha=0.05):
        ...
        min_p_value = min(p_values)
        max_p_value = max(p_values)
        file.write(f"Min p-value: {min_p_value}\n")
        file.write(f"Max p-value: {max_p_value}\n")
        
        plt.hist(p_values, bins=10, edgecolor='k')
        plt.xlabel('p-values')
        plt.ylabel('Frequency')
        plt.title('Histogram of p-values')
        plt.savefig(f'histogram_linked.png')
        plt.close()
        file.write(f"Histogram saved as histogram_linked.png\n")
        
        significant_tests = sum(p_value < alpha for p_value in p_values)
        empirical_power = significant_tests / X.shape[1]
        file.write(f"Nombre de tests significatifs: {significant_tests}\n")
        file.write(f"La puissance empirique du test est: {empirical_power:.2f}\n")
    ...
    file.write("\n--- Simulation pour Xj ∼ 0.7N(3Y,1) + 0.3N(0,1) ---\n")
    ...
    file.write("\n--- Simulation pour Xj ∼ 0.7N(2Y,1) + 0.3N(0,1) ---\n")
    ...
    file.write("\n--- Simulation pour Xj ∼ 0.7N(Y,1) + 0.3N(0,1) ---\n")
    ...
    file.write("\n--- Simulation pour Xj ∼ 0.3N(3Y,1) + 0.7N(0,1) ---\n")
    ...
    file.write("\n--- Simulation pour Xj ∼ 0.3N(2Y,1) + 0.7N(0,1) ---\n")
    ...
    file.write("\n--- Simulation pour Xj ∼ 0.3N(Y,1) + 0.7N(0,1) ---\n")
    ...
    file.write("\n--- Calcul des valeurs-p pour les covariables liées ---\n")
    p_values_combined = test_linked_covariates(X_combined, Y)
    file.write("\n--- Calcul des valeurs-p pour les covariables non liées ---\n")
    ...
    file.write(f"Min p-value (all): {min_p_value_all}\n")
    file.write(f"Max p-value (all): {max_p_value_all}\n")
    ...
    plt.close()
    file.write("Histogram saved as histogram_all.png\n")
    ...
    for method in methods:
        ...
        file.write(f"Method: {method}, Min p-value: {min_pval_corr}, Max p-value: {max_pval_corr}\n")
        ...
        plt.close()
        file.write(f"Histogram saved as histogram_adjusted_{method}.png\n")